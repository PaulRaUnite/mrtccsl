(*
   ...
   https://github.com/tautologico/ocaml-distrib/blob/master/matrix.ml
*)
...
let create ?(initval = 0.0) rows cols =
  { rows; cols; entries = Array.make (rows * cols) initval }
;;
...
let from_array ~rows ~cols a =
  if Array.length a != rows * cols
  then raise Incompatible_dimensions
  else { rows; cols; entries = a }
;;

let init ~rows ~cols ~f =
  { rows; cols; entries = Array.init (rows * cols) (fun i -> f (i / cols) (i mod cols)) }
;;

let init_vector n f = { rows = n; cols = 1; entries = Array.init n f }

let identity n =
  { rows = n
  ; cols = n
  ; entries = Array.init (n * n) (fun i -> if i mod (n + 1) = 0 then 1.0 else 0.0)
  }
;;

let rows m = m.rows
let columns m = m.cols
let vector_size v = if v.cols != 1 then raise Incompatible_dimensions else v.rows
let get m row col = m.entries.((row * m.cols) + col)
let set m row col value = m.entries.((row * m.cols) + col) <- value
let vector_from_array a = { rows = Array.length a; cols = 1; entries = a }
let copy m1 = { rows = m1.rows; cols = m1.cols; entries = Array.copy m1.entries }
...
let scmult m x = init ~rows:m.rows ~cols:m.cols ~f:(fun r c -> get m r c *. x)
...
let exists ~p m =
  let rec loop i =
    if i = m.rows * m.cols then false else if p m.entries.(i) then true else loop (i + 1)
  in
  loop 0
;;

let all ~p m = not (exists ~p:(fun x -> not (p x)) m)
