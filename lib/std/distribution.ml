(*
  ...
  https://github.com/tautologico/ocaml-distrib/blob/master/distribution.ml
...
let stdnorm_samples smp num =
  let res = Array.init num (fun _ -> sample_stdnormal_boxmuller3 smp) in
...
let samples_uni_vec  d n =
  Matrix.init_vector n (fun _ -> sample_uni d)
