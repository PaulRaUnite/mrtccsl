...
module Make (N : Num) = struct
  ...
  let of_policy name trigger =
    ...
    function
    | `AbsoluteTimer (period, { bounds; dist }) ->
      ( [ jitter_var, dist ]
      , { constraints =
            [ AbsPeriodic
                { out = trigger
                ; period = Const period
                ; error = jitter_var
                ; offset = Const N.zero (*TODO: make offset variable too.*)
                }
            ]
        ; var_relations = bounds_to_rel jitter_var bounds
        } )
    | `CumulativeTimer { bounds; dist } ->
      ( [ period_var, dist ]
      , { constraints =
            [ CumulPeriodic { out = trigger; period = period_var; offset = Const N.zero }
            ]
  ...
  let of_system ~relaxed_sched ?delayed_comm ?cores ((components, hal) : _ system) =
    ...
    let shedule_service =
      if relaxed_sched then relaxed_scheduling else rigid_scheduling
    in
    let scheduling =
      components
      |> List.to_seq
