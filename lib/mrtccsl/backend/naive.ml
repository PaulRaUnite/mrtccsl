...
let%test_module _ =
  (module struct
    ...
    module NumStrat = Strategy.Num (A.N) (A.NI)
    module LabelStrat = Strategy.Solution (A.L) (A.NI)

    let step = 0.1

    let slow_strat =
      LabelStrat.first @@ NumStrat.slow ~upper_bound:2.0 ~ceil:(Float.round_up step)
    ;;
